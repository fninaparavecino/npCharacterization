...
void fibGPU(int n, long int* arrayN)
{
  ...
  cudaErrCheck(cudaMalloc((void**)&devArrayN, sizeof(long int)*(n+1)));

  // cuda memcopy
  cudaErrCheck(cudaMemcpy(devArrayN, arrayN, sizeof(long int)*(n+1), cudaMemcpyHostToDevice));
  ...
  // retrieve results
  cudaErrCheck(cudaMemcpy(arrayN, devArrayN, sizeof(long int)*(n+1), cudaMemcpyDeviceToHost));
